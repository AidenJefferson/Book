...
public:
	...
	Vector_ref(T* a=0, T* b=0, T* c=0, T* d=0)
	{
			if (a) push_back(a);
			if (b) push_back(b);
			if (c) push_back(c);
...
struct Rectangle : Shape {
	...
	Rectangle(Point xy, int ww, int hh) :w(ww), h(hh) //, fcolor(0)
	{
		add(xy);
		if (h<=0 || w<=0) error("Bad rectangle: non-positive side");
	}
	Rectangle(Point x, Point y) :w(y.x-x.x), h(y.y-x.y) //, fcolor(0)
	{
		add(x);
